...
<#
.SYNOPSIS
  Safely uninstalls prometheus_windows_exporter from remote Windows hosts.
.DESCRIPTION
...
.EXAMPLE
  .\Uninstall-WindowsExporterRemote.ps1 -Computers SERVER01 -WhatIf
.EXAMPLE
  .\Uninstall-WindowsExporterRemote.ps1 -Computers SERVER01 -KeepFiles
#>
...
foreach ($computer in @($Computers | ForEach-Object { $_.Trim() } | Where-Object { $_ })) {
    ...
    $session = $null
    try {
        if (-not $PSCmdlet.ShouldProcess($computer, "Uninstall $ServiceName")) {
            $row.Service='WhatIf'; $row.Files='WhatIf'; continue
        }
        ...
        $result = Invoke-Command -Session $session -ScriptBlock {
            ...
            $parameters = Join-Path $serviceKey 'Parameters'
            ...
            if (-not $root) { $root = 'C:\Program Files\Observability\PrometheusExporters\windows-exporter' }
